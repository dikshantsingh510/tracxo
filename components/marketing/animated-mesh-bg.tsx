...
export function AnimatedMeshBg({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-[0.18] [background-image:linear-gradient(to_right,theme(colors.neutral.300/.5)_1px,transparent_1px),linear-gradient(to_bottom,theme(colors.neutral.300/.5)_1px,transparent_1px)] [background-size:48px_48px] [mask-image:radial-gradient(ellipse_70%_60%_at_50%_30%,black_30%,transparent_75%)] dark:opacity-[0.08]" />
      {/* Orbs */}
      <div className="-top-32 -left-24 absolute size-[28rem] animate-mesh-drift-1 rounded-full bg-emerald-300/45 blur-3xl dark:bg-emerald-700/35" />
      <div className="-right-40 absolute top-20 size-[34rem] animate-mesh-drift-2 rounded-full bg-teal-300/35 blur-3xl dark:bg-teal-800/30" />
      <div className="-bottom-32 absolute left-1/3 size-[30rem] animate-mesh-drift-3 rounded-full bg-emerald-500/30 blur-3xl dark:bg-emerald-600/20" />
    </div>
  );
}
